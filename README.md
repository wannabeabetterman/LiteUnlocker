...
2. **反作弊检测**：注入和 Hook 行为本身可被反作弊检测到，**不保证不被检测**。
...
2. **FPS 解锁**：`hk_GetFrameCount` 即游戏的 `Application.targetFrameRate` getter，解锁时直接返回目标帧率；`hk_ChangeFov` 每帧另调 `SetFrameCount` 双重保障（7.0 起原仓库已废弃旧的"钳回 60/45/30"做法）
3. **FOV 修改**：`hk_ChangeFov` 拦截 ChangeFOV 调用，把 value 参数改写成用户值后传给原函数
4. **FOV 平滑过渡**：用 `g_CurrentFov` 全局变量每帧做线性插值（指数衰减）逼近目标值，切场景时丝滑拉开而非瞬切
5. **特殊镜头保护**：FOV 修改只在 `value > 30` 时生效——队伍特写/过场动画时游戏会传入窄 FOV（<30），此时不覆盖，避免镜头穿帮
6. **移除队伍动画**：`hk_OpenTeam` 拦截打开队伍的调用，改为直接调 `OpenTeamPage(false)` 跳过角色展示动画
7. **隐藏 UID 水印**：主循环每 2 秒通过 `FindString + FindGameObject` 找到屏幕 UID 水印的 UI 对象，调 `SetActive(obj, false)` 隐藏。仅改本地显示，服务器侧真实 UID 不变
8. **关闭场景雾效**：`hk_DisplayFog` 拦截雾效函数，把雾参数复制到对齐安全缓冲后将"雾开关"标志位置 0，再用改造参数调原函数
9. **关闭角色半透明**：`hk_PlayerPerspective` 拦截角色透视函数，将透明度参数强制设为 1.0（完全不透明）
10. **随身合成台**：双路径设计。路径A——热键线程设置 `g_RequestCraft` 标志，主循环检测到后在游戏主线程调用 `CraftPartner` 打开合成界面（跨线程标志转发，避免 UI 跨线程崩溃）；路径B——`hk_CraftEntry` 拦截游戏原本的合成台入口，绕过位置检查直接打开界面

## 已知限制

- **特征码会随游戏版本失效**：特征码对应特定游戏版本，游戏更新后可能需要重新逆向
- 当前特征码已适配游戏 **7.0** 版本（与原仓库最新版同步，2026-08）
